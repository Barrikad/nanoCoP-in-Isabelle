...
fun clause_elem_size and mat_elem_size where
  ...
  \<open>clause_elem_size (Mat (Insert C Cs)) = mat_elem_size C + clause_elem_size (Mat Cs)\<close> |
  \<open>mat_elem_size (Cls _ \<lbrace>\<rbrace>) = 1\<close> |
  \<open>mat_elem_size (Cls idC (Insert M Ms)) = clause_elem_size M + mat_elem_size (Cls idC Ms)\<close>
...
1: \<open>
  ...
  b_clause (pol, pred, trms) (_,C) (_,C |-| \<lbrace>(Lit pol pred trms)\<rbrace>)\<close>|
2: \<open>
  b_clause L C' Cb \<Longrightarrow> 
  C' |\<in>| M \<Longrightarrow> 
  (Mat M) |\<in>| C \<Longrightarrow> 
  b_clause L (_,C) (_,finsert (Mat \<lbrace>Cb\<rbrace>) (C |-| \<lbrace>Mat M\<rbrace>))\<close>
...
proof-
  ...
  proof-
    have \<open>vars_in_mat \<lbrace>?C0\<rbrace> = \<lbrace>\<rbrace>\<close> sledgehammer
